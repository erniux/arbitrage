...
class CoinbaseClient:
    def __init__(self, public_key, secret_key, testnet):
        ...
        if testnet:
            ...
            self._wallet_base_url = 'https://api.coinbase.com/v2/'
    ...
    def _make_request(self, method, endpoint, data):
        if method == 'GET':
            try:
                ...
                logger.error("Connection error wjile making %s request to %s: %s", method, endpoint, e)
                return None
    ...
    def order_status(self, contract: Contract, order_id):
        data = dict()
        data['timestamp'] = int(time.time() * 1000)
        data['symbol'] = contract.symbol
        data['orderId'] = order_id
    ...
    def _start_ws(self):
        ...
        while True:
            try:
                ...
                logger.error("Error while starting websocket %s: %s", self._wss_url, e )
            time.sleep(2)
